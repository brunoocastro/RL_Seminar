...
class Sarsa:
    ...
    SARSA is an on-policy temporal difference (TD) learning algorithm that learns
    the Q-value function by updating based on the action actually taken by the
    current policy, rather than the greedy maximum action. This makes SARSA more
    conservative and directly dependent on the policy being followed.

    Key difference from Q-Learning:
    - Q-Learning (off-policy): Uses max Q(s',a') for updates (greedy evaluation)
    - SARSA (on-policy): Uses actual Q(s',a') where a' is chosen by current policy

    This makes SARSA safer in stochastic environments as it accounts for the
    exploration policy during learning, leading to more conservative value estimates.

    Attributes:
        learning_rate (float): Learning rate (alpha).
        gamma (float): Discount factor.
        explorer (ExplorerStrategy): Exploration strategy.
    """
    ...
    def update(self, state, action, reward, new_state, new_action):
        ...
        Unlike Q-Learning which uses max Q(s',a'), SARSA uses the actual next
        action a' that will be taken according to the current policy. This makes
        it an on-policy algorithm.

        Args:
            state: Current state
            action: Action taken in current state
            reward: Reward received
            new_state: Next state reached
            new_action: Next action that will be taken (chosen by policy)

        Returns:
            float: Updated Q-value for Q(s,a)
        """
        delta = (
            reward
    ...
    def train(self, env: Env, params: TrainingConfig):
        """Train the SARSA agent.

        The training loop follows the SARSA pattern:
        1. Initialize first action before the episode loop
        2. Take action, observe next state and reward
        3. Choose next action based on next state (before updating)
        4. Update Q(s,a) using the actual next action a'
        5. Move to next state and reuse the already-chosen next action

        This pattern ensures that the update uses the action that will actually
        be taken, maintaining the on-policy nature of SARSA.
        ...
        for run in range(
            ...
            render_this_run = (
                params.render_each > 0 and (run + 1) % params.render_each == 0
            )
            # We'll render only a single episode for the run (use the last episode of the run)
            render_episode_index = (
                params.total_episodes - 1 if render_this_run else None
            )
            ...
            for episode in tqdm(
                ...
                # SARSA: Initialize first action before the loop
                ...
                while not done:
                    ...
                    new_action = self.explorer.choose_action(
                    ...
                    # Move to next state and reuse the already-chosen action
                    state = new_state
                    action = new_action
                        ...
                        try:
                            ...
                            for fr in frames:
                                ...
                                try:
                                    delay = (
                                        params.render_delay
                                        if hasattr(params, "render_delay")
                                        else 0.05
                                    )
                                    plt.pause(delay)
                                except Exception:
    ...
    def save_qtable(self, size=(4, 4), name_prefix="sarsa_qtable"):
    ...
    def load_qtable(self, size=(4, 4), name_prefix="sarsa_qtable"):
